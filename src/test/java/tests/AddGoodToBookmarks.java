...
public class AddGoodToBookmarks extends BaseTest {
    ...
    public void additionalBookmarks() throws IOException, InterruptedException {

        GoodsPage goodsPage = new LoginPage(BaseTest.driver).get().login().openGoodsPage();
        ...
        new Actions(BaseTest.driver).sendKeys(Keys.PAGE_UP).sendKeys(Keys.PAGE_UP).build().perform();
        ...
        Assertions.assertAll("Matching of products selected on the main page and added to bookmarks",
                () -> assertThat(nameGoods).as("проверка идентичности имени").isEqualTo(nameItem),
                () -> assertThat(priceGoods).as("проверка идентичности цены товара").isEqualTo(priceItem));
